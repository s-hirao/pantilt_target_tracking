...
class pantiltPositionControlClass{
	...
	// input_u
	Eigen::Vector3d vel_c_t;//命令速度今は直進のみ制御入力
	Eigen::Vector3d omega_c_psi;//命令角度パンチルトのみ角度入力
	// control_target
	Eigen::Vector3d err_vel_c_t;//命令速度今は直進のみ誤差の微分
	Eigen::Vector3d err_omega_c_psi;//命令角度パンチルトのみ誤差の微分
    public:
...
void pantiltPositionControlClass::targetDecision(){
		...
		// 検出2個目から一番距離が近い障害物をgcMinに入れる
			...
			//特定したクラスタを障害物としてその重心からの幅計算
 			for(k = 0; k < (int)copyTargetGravityCenterMessage.data[l].pt.size() ; k++){
				...
				length = sqrt((copyTargetGravityCenterMessage.data[l].pt[k].y - copyTargetGravityCenterMessage.data[l].gc.y)
							*(copyTargetGravityCenterMessage.data[l].pt[k].y - copyTargetGravityCenterMessage.data[l].gc.y));
    			// 全体の重心から最も広い幅,最も狭い奥行きをwidthMax,lengthMaxに入れる
				if(width >= widthMax){
    			    widthMax = width;
    			}
				if(length >= lengthMax ){
	...
	// 対象の幅と奥行きを考慮した距離の範囲内で終了 
	// ここでの止まる対象はカメラ座標で目標値についたらなのでロボット座標で直進方向はstop()させる
	widthMax = widthMax ;
	lengthMax = lengthMax ;
...
void pantiltPositionControlClass::stop(){	
		err_vel_c_t = Eigen::Vector3d::Zero();//ビーゴ停止
...
void pantiltPositionControlClass::calculationProcess(){
	...
	RotationToPWM();
	angularVelocityCopy();
	//後ろに進まないこと、0.1m/s以下では動けない publish判断	
	if(err_vel_c_t(1) >= 0){
		if(err_vel_c_t(1) < 1.0){
		publishJudgment();
	 	}
		if(err_vel_c_t(1) <= 0.01 ){
		stop();	
	 	} 	
		if(err_vel_c_t(1) >= 1.0 ){
		ROS_INFO("no_publish");
...
void pantiltPositionControlClass::sensorDataGet(){
	...
	psi_o_c(2) = atan2(t_o_c(0) , t_o_c(1))*(180/M_PI);//ヨー 目標までのズレ 相対姿勢　rad
	ROS_INFO("psi_atan2= %f" ,psi_o_c(2));
...
void pantiltPositionControlClass::deviation(){
	//偏差 観測データと目標値の差
	// 逆かも
	error_difference  = -1*(t_g_c - t_o_c) ;//並進
	error_theta 	  = -1*(phi_g_c - psi_o_c) ;//回転
	ROS_INFO("diffe_x = %f" ,error_difference(0));
	ROS_INFO("diffe_y = %f" ,error_difference(1));
	ROS_INFO("diffe_z = %f" ,error_difference(2));
	ROS_INFO("theta_x = %f" ,error_theta(0));
	ROS_INFO("theta_y = %f" ,error_theta(1));
	ROS_INFO("theta_z = %f" ,error_theta(2));
...
void pantiltPositionControlClass::input(){
	// -λ逆行列J-1誤差e
 	vel_c_t = -1*lambda*(error_difference + t_o_c.cross(error_theta));
	...
	ROS_INFO("rot_z = %f" ,omega_c_psi(0));	
	ROS_INFO("rot_z = %f" ,omega_c_psi(1));
	...
	ROS_INFO("rot_z = %f" ,err_omega_c_psi(0));
	ROS_INFO("rot_z = %f" ,err_omega_c_psi(1));
	ROS_INFO("rot_z = %f" ,err_omega_c_psi(2));
	ROS_INFO("input_u_ok");
}
// ビーゴ右左合わせての速度publish
void pantiltPositionControlClass::beegoVelocityOrder(){
	// ビーゴ命令値
	beegoMessage.linear.x = err_vel_c_t(1);
	beegoMessage.linear.y = err_vel_c_t(0);
	beegoMessage.linear.z = err_vel_c_t(2);
	ROS_INFO("vel_x = %f" ,err_vel_c_t(0));
	ROS_INFO("vel_y = %f" ,err_vel_c_t(1));
	ROS_INFO("vel_z = %f" ,err_vel_c_t(2));
}
void pantiltPositionControlClass::RotationToPWM(){
	...
	//絶対値を取る
	pwmRotationPitch = abs(err_omega_c_psi(1));
...
,time(ros::Time::now()){
	...
	err_vel_c_t = Eigen::Vector3d::Zero(3);
	err_omega_c_psi = Eigen::Vector3d::Zero(3);	
...
int main(int argc, char **argv){
	ros::init(argc,argv,"pantilt_position_control_node");//実行ノード名
